...
# Run with the built-in demo config (no config file needed — shows example tiles):
python -m f5authtester
# → http://127.0.0.1:8080
```

Point it at your environment by copying the example config and referencing it:

```bash
cp config.example.yaml config.yaml
# edit config.yaml, then:
export F5AUTHTESTER_CONFIG=./config.yaml
export F5_PASSWORD='…'          # secrets come from the environment, never the file
python -m f5authtester
```
